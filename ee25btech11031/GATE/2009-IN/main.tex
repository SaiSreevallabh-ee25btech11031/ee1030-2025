...
\section*{\textbf{Q.1-Q.20 Carry one mark each}} % the '*' is so that we do not get bullet number for the section heading

\vspace{0.5cm} %vertical space that must be left
...
\vspace{0.5cm}
%7
\item The diodes shown in the circuit ($\figref{fig:placeholder_2}$) are ideal. A voltage of $0V$ represents logic $0$ and $+5V$  represents logic $1$. The logic function Z realied by the circuit for logic inputs X and Y is\par  \hfill\brak{\text{GATE IN 2009}}
\begin{figure}[H]
    \centering
...
\begin{multicols}{4}
    \begin{enumerate}
        ...
        \item $\frac{1}{\sqrt{2}}\cos{(t-\frac{\pi}{4})}$
...
\vspace{0.5cm}
%21
\item The eigenvalues of a $(2\times2)$ matrix \textbf{X} are $-2$ and $-3$. The eigenvalues of the matrix $(\textbf{X}+\textbf{I})^{-1}(\textbf{X}+5\textbf{I})$ are \par \hfill{\brak{\text{GATE IN 2009}}}
\begin{multicols}{4} 
    \begin{enumerate}
        \item $-3, -4$
        \item $-1,-2$
...
\vspace{0.5cm}
%25
\item The differential equation $\dfrac{dx}{dt}=\frac{4-x}{\tau}$ with $x\brak{0}=0$, and the constant $\tau>0$, is to be numerically integrated using the forward Euler method with a constant integration time step $T$. The maximum value of $T$ such that the numerical solution of $x$ converges is \hfill{\brak{\text{GATE IN 2009}}}
\begin{multicols}{4}
    \begin{enumerate}
        \item ${\tau}/{4}$
        \item $\tau/2$
...
%42
\item The transfer function $H\brak{z}$ of a fourth-order linear phase FIR system is given by\\ $H(z)=(1+2z^{-1}+3z^{-2})G(z)$.\\
Then $G(z)$ is\par  \hfill{\brak{\text{GATE IN 2009}}}
\begin{multicols}{4}
    \begin{enumerate}
        \item $3+2z^{-1}+z^{-2}$
...
\vspace{0.1cm}
\begin{align}
  \dfrac{d\textbf{x}}{dt} & =\textbf{Fx}+\textbf{G}u\\
y= & \textbf{Hx}
\end{align}
